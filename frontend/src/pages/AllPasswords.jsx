...
import { getPasswords, createPassword, updatePassword, deletePassword } from '../api/passwords'
...
const AllPasswords = () => {
    ...
    const handleSave = async (entry) => {
        try {
            const newEntry = await createPassword({ ...entry, type: 'password' })
    ...
    return (
        <div className='min-h-screen bg-black'>
            ...
            <h1 className='text-4xl font-bold text-white px-4 mb-2'>All</h1>

            <SearchBar value={search} onChange={(e) => setSearch(e.target.value)} />

            <PasswordList passwords={passwords} search={search} onDelete={handleDelete} onEdit={handleEdit} />

            <p className='text-center text-gray-600 text-sm mt-4'>{passwords.length} Items</p>
